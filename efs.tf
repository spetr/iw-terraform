...
resource "aws_efs_mount_target" "data" {
  for_each        = var.ec2_instance_count <= 1 ? { "0" = aws_subnet.private["0"] } : aws_subnet.private
...
resource "aws_efs_mount_target" "archive" {
  for_each        = var.enable_efs_archive ? (var.ec2_instance_count <= 1 ? { "0" = aws_subnet.private["0"] } : aws_subnet.private) : {}
